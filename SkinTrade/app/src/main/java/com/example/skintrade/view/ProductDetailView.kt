...
fun ProductDetailView(product: Product, onBackClicked: () -> Unit, onAddToCartClicked: () -> Unit) {
    Scaffold(
        ...
        Column(
            ...
            Text(product.description, color = Color.LightGray, fontSize = 16.sp, modifier = Modifier.fillMaxWidth())
            ...
            when (product) {
                ...
                is Agent -> DetailRow("Facción:", product.category)
                is Case -> DetailRow("Contenido Destacado:", product.featuredContent)
                is Soundtrack -> DetailRow("Autor:", product.author)
            }
